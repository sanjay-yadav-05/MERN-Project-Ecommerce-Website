              ...
              <div className="overflow-auto grid grid-cols-5 auto-rows-[48%] gap-4 h-full p-2 w-full">
                {filteredProducts.length > 0 ? (
                  filteredProducts.map((product) => (
                    <div key={product._id} onClick={openProduct(product)} className="relative border h-full w-full py-2 rounded-md overflow-hidden hover:bg-gray-200">
                      ...
                      <div className="flex flex-col gap-0.5 w-[88%] m-auto h-full ">
                        <img
                          src={`http://localhost:8080/api/v1/product/get-product-image/${product._id}`}
                          alt={product.name}
                          className="h-[60%] object-cover rounded-lg mb-1"
                        />
                        ...
                        <div className='w-full flex justify-between'>
                          <button className='bg-slate-500 font-semibold text-white  text-sm px-2 py-1 rounded-md'>More Info</button>
                          <button className='bg-slate-500 font-semibold text-white text-sm px-2 py-1 rounded-md'>Add to Cart</button>
                        </div>
                      </div>
                    </div>
                  ))
