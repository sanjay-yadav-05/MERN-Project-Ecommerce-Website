...
//           <input type="text" placeholder='sreach product' />
//         </nav>
//         {auth.user ? (
...
const Header = () => {
  ...
  return (
    <header className="fixed top-0 w-full bg-black text-white shadow-xl shadow-gray-500 md:shadow-gray-500 z-10">
      {/* Desktop Navigation */}
      <div className="hidden md:flex justify-between items-center h-[8vh] px-6">
        <NavLink to="/" className="text-4xl w-[35%]">BrandName</NavLink>
        {isHomePage && <nav className="flex gap-6 w-[30%] box-border  justify-center items-center h-full relative">
          <input
            type="text"
            placeholder="Search products"
            value={searchQuery}
            onChange={handleSearchChange}
            className="px-3 py-2 w-full text-black m-auto text-center absolute rounded-lg border border-gray-400"
          />
          {/* Display search suggestions */}
          {searchQuery && filteredProducts.length > 0 && (
            <div className="absolute bg-white text-black border top-full rounded-lg w-full max-h-60 overflow-y-auto shadow-lg z-10">
        ...
        {auth.user ? (
          <div className="flex gap-4 bg-black items-center w-[35%] justify-end relative">
            {/* Profile Section */}
            <div className="overflow-hidden">
              <div className={`flex gap-2 h-full z-40 ease-in-out cursor-pointer duration-500 ${isDropdownOpen ? "translate-x-0" : "translate-x-[200%]"}`}>
                <NavLink to={`/dashboard/${auth.user.role === 1 ? "admin" : "user/profile"}`} className="hover:bg-gray-800 px-2 rounded-l-lg py-2" onClick={closeDropdown}>
